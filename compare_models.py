#!/usr/bin/env python3
"""
Compare Models: Ratio vs Asymmetric
...
from load_asymmetric_model import (
    load_asymmetric_model, 
    predict_densities_and_ratio_asymmetric,
    calculate_precision_from_asymmetric_predictions
)
...
def predict_average_precision_asymmetric_model(model, alerts_df, k):
    """Predict average precision using asymmetric model"""
...
def main():
    print("🔬 COMPARING RATIO vs ASYMMETRIC MODELS")
    print("=" * 50)
    
    # Load models
    print("Loading models...")
    ratio_model, _ = load_best_model_with_ratio()  # Unpack the tuple
    asymmetric_model = load_asymmetric_model()
    ...
    for prop in proportions:
        ...
        for i, camera_data in enumerate(tqdm(test_cameras, desc=f"Prop {prop}")):
            try:
                ...
                # Asymmetric model prediction
                asymmetric_ap = predict_average_precision_asymmetric_model(asymmetric_model, alerts_df, k)
                ...
                prop_results.append({
                    ...
                    'gt_ap': gt_ap,
                    'ratio_ap': ratio_ap,
                    'asymmetric_ap': asymmetric_ap,
                    'ratio_error': abs(ratio_ap - gt_ap),
                    'asymmetric_error': abs(asymmetric_ap - gt_ap),
                    'improvement': abs(asymmetric_ap - gt_ap) - abs(ratio_ap - gt_ap)  # Negative = better
                })
        ...
        if prop_results:
            ...
            avg_asymmetric_error = np.mean([r['asymmetric_error'] for r in prop_results])
            ...
            print(f"   Asymmetric Model Error: {avg_asymmetric_error:.4f}")
    ...
    if results:
        ...
        if overall_improvement_pct < 0:
            print("✅ ASYMMETRIC model improved precision accuracy!")
            print("🎉 Your hypothesis about biasing ratios was CORRECT!")
        else:
            print("❌ ASYMMETRIC model made things worse")
            print("🤔 May need different asymmetric penalty ratios")
...
def create_comparison_plot(results, proportions):
    ...
    for prop in proportions:
        ...
        if prop_data:
            ...
            asymmetric_errors.append(np.mean([r['asymmetric_error'] for r in prop_data]))
    ...
    ax1.bar(x + width/2, asymmetric_errors, width, label='Asymmetric Model', color='orange', alpha=0.7)
    ...
    ax2.set_title('Asymmetric Model Improvement\n(Negative = Better)')
    ...
    plt.tight_layout()
    plt.savefig('model_comparison_ratio_vs_asymmetric.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"\n📊 Comparison plot saved to model_comparison_ratio_vs_asymmetric.png")
